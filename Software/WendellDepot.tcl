#!/usr/bin/wish
# Generated code: Wed Feb 26 14:28:22 EST 2025
...
# OpenLCB_Dispatcher Stage_West_2_4_Plate -eleclasstype SwitchPlate -normaleventid 05.01.01.01.22.83.00.32 -reverseeventid 05.01.01.01.22.83.00.33 -normalindonev 05.01.01.01.22.83.00.52 -normalindoffev 05.01.01.01.22.83.00.53 -reverseindonev 05.01.01.01.22.83.00.53 -reverseindoffev 05.01.01.01.22.83.00.52
OpenLCB_Dispatcher create %AUTO% -name Stage_West_2_4_Plate \
	...
	-normalindonev [lcc::EventID %AUTO% -eventidstring {05.01.01.01.22.83.00.52}] \
	-normalindoffev [lcc::EventID %AUTO% -eventidstring {05.01.01.01.22.83.00.53}] \
	-reverseindonev [lcc::EventID %AUTO% -eventidstring {05.01.01.01.22.83.00.53}] \
	-reverseindoffev [lcc::EventID %AUTO% -eventidstring {05.01.01.01.22.83.00.52}]
...
# OpenLCB_Dispatcher Stage_West_2_4 -eleclasstype Switch -statenormaleventid 05.01.01.01.22.83.00.52 -statereverseeventid 05.01.01.01.22.83.00.53
OpenLCB_Dispatcher create %AUTO% -name Stage_West_2_4 \
	-eleclasstype Switch \
	-statenormaleventid [lcc::EventID %AUTO% -eventidstring {05.01.01.01.22.83.00.52}] \
	-statereverseeventid [lcc::EventID %AUTO% -eventidstring {05.01.01.01.22.83.00.53}]
...
snit::type DispatcherLogic {
    ...
    method _sigPlateLeft {sname} {
        ...
        MainWindow ctcpanel seti $sname L yes
        MainWindow ctcpanel seti $sname C no
        MainWindow ctcpanel seti $sname R no
    ...
    method _sigPlateCenter {sname} {
        ...
        MainWindow ctcpanel seti $sname L no
        MainWindow ctcpanel seti $sname C yes
        MainWindow ctcpanel seti $sname R no
    }
    method _sigPlateRight {sname} {
        ...
        MainWindow ctcpanel seti $sname L no
        MainWindow ctcpanel seti $sname C no
        MainWindow ctcpanel seti $sname R yes
    }
    method sendevent {eopt} {
        set ev [$self cget $eopt]
        if {$ev ne ""} {
    ...
    method consumeEvent {event} {
        foreach os [array names opticalSensors] {
            lassign $opticalSensors($os) name on off
            if {[$on match $event]} {
                #puts stderr "*** $self consumeEvent: $name covered"
                ...
                #puts stderr "*** $self consumeEvent: dir is $dir"
                if {$dir in {Left Right}} {
                    lassign [dict get $sensedict $dir] otherend blocks
                    #puts stderr "*** $self consumeEvent: otherend is $otherend, blocks is $blocks"
                    foreach b $blocks {
                        set occupied($b) yes
                    ...
                    foreach o $otherend {
                        set sensorState($o) $blocks
                    }
                }
                break
            } elseif {[$off match $event]} {
                #puts stderr "*** $self consumeEvent: $name uncovered"
                if {[info exists sensorState($os)]} {
                    set blocks $sensorState($os)
                    #puts stderr "*** $self consumeEvent: blocks is $blocks"
                    unset sensorState($os)
                    foreach b $blocks {
                        set occupied($b) no
                        MainWindow ctcpanel invoke $b
                    }
